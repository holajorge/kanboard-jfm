...
class TaskHelper extends Base
{
    ...
    public function renderInputOneField(array $values, array $errors = array(), array $attributes = array())
    {
        $attributes = array_merge(array('tabindex="16"'), $attributes);

        $html = $this->helper->form->label('hola', 'hola');
        $html .= $this->helper->form->text('hola', $values, $errors, $attributes, 'form-input-small');

        return $html;
    }
    public function renderInputTwoField(array $values, array $errors = array(), array $attributes = array())
    {
        $attributes = array_merge(array('tabindex="17"'), $attributes);

        $html = $this->helper->form->label('mundo', 'mundo');
        $html .= $this->helper->form->text('mundo', $values, $errors, $attributes, 'form-input-small');
